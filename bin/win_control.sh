...
TOPIC="cortex_alerta_anna_armengol"
...
if (( $(echo "$TEMP > 32.0" | bc -l) )); then
    ...
    curl -s -H "Title: CORTEX BIOTOPO" -H "Priority: urgent" -d "$MSG" "https://ntfy.sh" > /dev/null || true
elif (( $(echo "$TEMP < 24.0" | bc -l) )); then
    MSG="CRITICAL - Deficit termico en el biotopo de Sefa: $TEMP C."
    echo "$(date -Iseconds) - $MSG" >> "$LOG_FILE"
    curl -s -H "Title: CORTEX BIOTOPO" -H "Priority: high" -d "$MSG" "https://ntfy.sh" > /dev/null || true
fi
